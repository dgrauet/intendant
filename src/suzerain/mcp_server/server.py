"""FastMCP server exposing suzerain governance tools over stdio.

Run via ``suzerain mcp`` (see :mod:`suzerain.commands.mcp`). Requires the
optional ``mcp`` extra: ``uv tool install 'suzerain[mcp]'``.
"""
...
from suzerain.mcp_server import handlers

mcp = FastMCP("suzerain")


@mcp.tool()
def audit_repo(path: str, severity: str | None = None) -> dict[str, Any]:
    """Audit a single repo against its `.suzerain.toml` configuration.
