...
:mod:`intendant.mcp_server.server`. Each returns either a successful payload
...
from intendant.audit.output.json_format import _build_payload
...
from intendant.audit.snapshot import find_latest_snapshot, load_snapshot
...
def audit_repo(path: str, severity: str | None = None) -> dict[str, Any]:
    """Audit a single repo. Returns the same JSON shape as `intendant audit --format=json`.
...
def report_portfolio(path: str, maxdepth: int = 2) -> dict[str, Any]:
    """Scan a portfolio root and return the same JSON as `intendant report --format=json`."""
    root = _resolve_existing_dir(path)
    if root is None:
        return _err(f"path not found or not a directory: {path}")
    try:
        # Lazy import to avoid circular: report imports from this module's siblings
        from intendant.commands.report import _scan_all
...
def diff_portfolio(path: str, against: str | None = None, maxdepth: int = 2) -> dict[str, Any]:
    ...
    If `against` is None, looks up the latest snapshot in `<path>/.intendant/snapshots/`.
    ...
    if against is not None:
        ...
        snapshot_dir = root / ".intendant" / "snapshots"
        ...
        if snapshot_path is None:
            return _err(
                ...
                "run `intendant report --save-snapshot` first or pass `against`"
            )
    try:
        from intendant.commands.report import _scan_all
