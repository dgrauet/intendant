...
def scaffold_project(target: Path, stack: str, context: SubstitutionContext) -> None:
    """Generate a new conformant intendant project at `target`.
    ...
    _strict_enforcement_in_intendant_toml(target, context)
...
def _strict_enforcement_in_intendant_toml(target: Path, context: SubstitutionContext) -> None:
    cfg = target / ".intendant.toml"
    if not cfg.is_file():
        return
    data = tomllib.loads(cfg.read_text(encoding="utf-8"))
    data.setdefault("intendant", {})
    data["intendant"]["enforcement"] = "strict"
    data["intendant"]["stack"] = context.stack
