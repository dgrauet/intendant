...
def collect_rules() -> list[Rule]:
    ...
    rules: list[Rule] = []
    try:
        from intendant.checks import RULES

        rules.extend(RULES)
    except ImportError:
        pass
    try:
        from intendant.adapters.python import RULES

        rules.extend(RULES)
    except ImportError:
        pass
    try:
        from intendant.adapters.claude_skill import RULES

        rules.extend(RULES)
    except ImportError:
        pass
    try:
        from intendant.adapters.node import RULES

        rules.extend(RULES)
    except ImportError:
        pass
    try:
        from intendant.adapters.rust import RULES
    ...
    try:
        ...
        pass
    try:
        from intendant.adapters.swift import RULES
...
def filter_for_repo(rules: Sequence[Rule], repo: Repo, config: IntendantConfig) -> list[Rule]:
