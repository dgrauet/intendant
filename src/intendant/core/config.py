"""Parser for .intendant.toml — per-repo governance configuration."""
...
@dataclass(frozen=True)
class IntendantConfig:
    """Parsed .intendant.toml content.
...
def load_config(repo_path: Path) -> IntendantConfig:
    """Load .intendant.toml from `repo_path`. Returns defaults if no file.
    ...
    """
    cfg_path = repo_path / ".intendant.toml"
    if not cfg_path.is_file():
        return IntendantConfig(version="1", stack=None, enforcement=DEFAULT_ENFORCEMENT)
    raw = tomllib.loads(cfg_path.read_text())
    suz = raw.get("intendant", {})
    if "mode" in suz:
        raise ValueError(
