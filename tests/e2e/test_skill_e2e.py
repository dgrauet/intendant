...
def test_e2e_valid_skill_passes_all_sk_rules(fixtures_dir: Path) -> None:
    repo_path = fixtures_dir / "valid_skill_repo"
    repo = Repo.from_path(repo_path)
    assert repo.stack == "skill"

    cfg = SuzerainConfig(version="1", stack="skill", mode="strict")
    report = run_audit(repo, cfg, SKILL_RULES)
