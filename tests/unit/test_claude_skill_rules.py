...
def test_sk001_passes_when_skill_md_exists(tmp_path: Path) -> None:
    from intendant.adapters.claude_skill.sk import SK001SkillMdExists
...
def test_sk002_passes_with_valid_frontmatter(tmp_path: Path) -> None:
    from intendant.adapters.claude_skill.sk import SK002FrontmatterValid
...
def test_sk003_passes_with_normal_description(tmp_path: Path) -> None:
    from intendant.adapters.claude_skill.sk import SK003DescriptionQuality
...
def test_sk004_passes_when_name_matches_dir(tmp_path: Path) -> None:
    from intendant.adapters.claude_skill.sk import SK004NameMatchesDir
...
def test_sk005_passes_when_evals_dir_has_files(tmp_path: Path) -> None:
    from intendant.adapters.claude_skill.sk import SK005EvalsNonEmpty
...
def test_sk006_passes_when_no_dir_mentioned(tmp_path: Path) -> None:
    from intendant.adapters.claude_skill.sk import SK006ReferencedDirsExist
...
def test_sk007_passes_when_readme_mentions_claude_skills_path(tmp_path: Path) -> None:
    from intendant.adapters.claude_skill.sk import SK007ReadmeInstallPath
