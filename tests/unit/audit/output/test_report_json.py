"""Unit tests for the report JSON formatter."""
...
def test_root_is_absolute_string(tmp_path: Path) -> None:
    scan = PortfolioReport(root=tmp_path, reports=[], timestamp=datetime(2026, 5, 2, 15, 42))
    parsed = json.loads(render_report_json(scan))
    assert parsed["root"] == str(tmp_path)


def test_timestamp_is_iso8601(tmp_path: Path) -> None:
    scan = PortfolioReport(root=tmp_path, reports=[], timestamp=datetime(2026, 5, 2, 15, 42, 0))
    parsed = json.loads(render_report_json(scan))
